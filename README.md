...
4. **Data actually loads when navigating the app.** In the browser, sign up for an account and browse the homepage, global feed and your profile. Open the browser's DevTools → Network tab while doing this to see the underlying `/api/...` requests succeed with `200` status codes. (Creating a new article currently fails — see [Known Issues](#known-issues).)
...
- Creating a new article from the frontend fails with a `404`. The original RealWorld frontend code sends `POST /articles/` (with a trailing slash) in `articles.service.ts`, while the backend's router is configured with `trailing_slash=False` and every other endpoint call in the frontend omits it — this one call is inconsistent with the rest of the codebase. Reading articles, tags, and profiles is unaffected, as is signing up and logging in. Fix: remove the trailing slash on that one call and rebuild the frontend image.
